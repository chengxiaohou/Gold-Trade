...
const CustomTagCard: React.FC<{ tag: UserTagRule; onEdit: () => void; onToggle: () => void; onDelete: () => void }> = ({ tag, onEdit, onToggle, onDelete }) => {
  ...
  return (
    <div
      className={`relative inline-block px-2.5 py-1 rounded-full text-xs border border-app-border text-app-subtext cursor-pointer transition-colors ${tag.enabled ? '' : 'opacity-70'}`}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      onClick={() => setPinned(p => !p)}
    >
      {/* 文字始终渲染用于撑开卡片固定尺寸；按钮以绝对定位覆盖其上居中，不影响卡片宽高 */}
      <span className="block leading-tight whitespace-nowrap">{desc}</span>
      {show && (
        <div className="absolute inset-0 flex items-center justify-center gap-0.5 bg-app-bg rounded-full">
...
const CLASSIC_PARAM_LABELS: Record<keyof TagParams['classic'], { label: string; desc: string }> = {
  ...
  classicVolHighStrong: { label: '明显放量阈值', desc: '当日量 ≥ 前5日均量 × 该值（默认1.4），判为“明显放量”' },
  classicVolHighMid: { label: '温和放量阈值', desc: '当日量 ≥ 前5日均量 × 该值（默认1.15）且 < 明显放量阈值，判为“温和放量”' },
  classicVolLowMid: { label: '温和缩量阈值', desc: '当日量 < 前5日均量 × 该值（默认0.8）进入缩量区间，判为“温和缩量”' },
  classicVolLowStrong: { label: '明显缩量阈值', desc: '当日量 ≤ 前5日均量 × 该值（默认0.55），判为“明显缩量”' },
};
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1">
                            ...
                            <input
                              ...
                              className="w-full h-1.5 bg-app-input rounded-lg appearance-none cursor-pointer accent-gray-500 
